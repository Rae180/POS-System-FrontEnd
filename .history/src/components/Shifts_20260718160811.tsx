...
(match the actual class names already used elsewhere in this file for consistency — I don't have the exact original className strings in front of me for the open/closed styling, so swap in whatever colors the rest of the file already uses for similar status badges, like the refunded/completed pattern in Orders.tsx)
Simpler alternative if you'd rather compute it once above the JSX instead of inline:
jsconst status = shift.closed_at ? 'closed' : 'open';
right where const diff = shift.variance ?? 0; already sits, then just reference {status} in the badge.
Worth doing once, not per-bug: a full grep for phantom fields
You've now hit this exact failure mode four separate times in this one table — expected_cash_so_far, variance/difference, and now status, plus opening_float typing earlier. That's not bad luck, it's a real signal: this table was written against an assumed API shape that was never checked against the actual backend response at build time. Worth one systematic pass instead of finding the fifth one by hand:
bashfindstr /n "shift\." src\components\Shifts.tsx
Cross-check every shift.X reference that comes up against the real fields your backend actually returns (id, opened_by, closed_by, opening_float, closing_cash_counted, opened_at, closed_at, notes, plus the two I just added: expected_cash_so_far, variance). Anything referenced that's not in that list is the next silent bug waiting to surface — better to catch it now in five minutes than one broken column at a time.
                      </td>
