...
export const Orders: React.FC = () => {
  ...
  const filteredOrders = orders.filter((o: any) => {
    ...
    const customerMatch = o.customer ? getFullName(o.customer).toLowerCase().includes(term) : false;
    const cashierMatch = o.user?.first_name?.toLowerCase().includes(term) || o.user?.last_name?.toLowerCase().includes(term);
                ...
                return (
                  <tr key={order.id} className="hover:bg-gray-50/50 dark:hover:bg-[#1b1b18]/40 transition">
                    ...
                    <td className="px-6 py-4">
                      {order.user ? `${order.user.first_name} ${order.user.last_name}` : 'System'}
                    </td>
                    <td className="px-6 py-4 font-semibold text-gray-800 dark:text-[#EDEDEC]">
                      {order.customer ? getFullName(order.customer) : 'Walk-In'}
                    </td>
                    <td className="px-6 py-4">
            ...
            >
              ...
              <div className="py-4 border-b border-dashed border-gray-300 space-y-1.5 text-[10px] text-gray-600">
                ...
                <div className="flex justify-between items-center pt-2 border-t border-dashed border-gray-200">
                  ...
                  <span>${selectedReceipt.amount_paid.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center text-xs font-bold text-gray-900">
                  <span>CHANGE GIVEN:</span>
                  <span>
                    {selectedReceipt.amount_paid > selectedReceipt.total 
                      ? `$${(selectedReceipt.amount_paid - selectedReceipt.total).toFixed(2)}` 
                      : '$0.00'}
                  </span>
                </div>
              </div>
              ...
              {isAdmin && selectedReceipt.status !== 'refunded' && (
                <div className="mt-4 pt-4 border-t border-dashed border-gray-300 print:hidden">
                  {refundingOrderId === null ? (
                    <button
                      onClick={() => setRefundingOrderId(selectedReceipt.order_id || null)}
